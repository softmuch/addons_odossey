...
const { BasePrinter } =
  import.meta.env.MODE !== 'development'
    ? odoo.require('@point_of_sale/app/printer/base_printer')
    : { BasePrinter: Object }
...
export class LocalPrinter extends BasePrinter {
   ...
   */
  // @ts-ignore
  async sendPrintingJob(imgBase64) {
    const img = document.createElement('img')
    img.src = `data:image/jpg;base64,${imgBase64}`
    await this.printElement(img)
  }

  async printElement(el: Element) {
    const w = window.open('', '__blank')

    if (w) {
      w.document.body.innerHTML = el.outerHTML
      w.document.close()
      // Ensure styles are applied and content is rendered before printing
      setTimeout(() => {
        w.focus()
        w.print()
