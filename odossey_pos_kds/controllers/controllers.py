...
class PosKitchenScreen(http.Controller):
    ...
    @http.route("/odossey_pos_kds/dashboard", auth="user", website=True)
    def dashbaord(self, **kw):
        return request.render("odossey_pos_kds.dashboard")

    @http.route("/odossey_pos_kds/app", auth="user")
    ...
    def render_vue_app(self, index_path: str, ks=False, **kw):
        ...
        redirect_url = '/web#action=odossey_pos_kds.action_kitchen_screen'
        if not ks:
        ...
        assets = http.request.render(
            "odossey_pos_kds.kitchen_assets", context
        ).render()
