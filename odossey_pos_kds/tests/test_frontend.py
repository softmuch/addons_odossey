...
class TestFrontend(AccountTestInvoicingCommon, HttpCaseWithUserDemo):
    ...
    def test_01_odossey_pos_kds(self):
        if not loaded_demo_data(self.env):
            _logger.warning("This test relies on demo data. To be rewritten independently of demo data for accurate and reliable results.")
            return 
        
        _logger.info("Running test_01_odossey_pos_kds")

        self.pos_config.with_user(self.pos_admin).open_ui()
        self.start_tour("/odossey_pos_kds/app/?config_id=%d" % self.pos_config.id, 'odossey_pos_kds_tour', login="pos_admin", watch=False)
